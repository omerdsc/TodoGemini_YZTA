...
from ..database import engine,SessionLocal #seccionlocal veritabanıyla bağlantı sağlar
from typing import Annotated
from starlette import status
from ..routers.auth import get_current_user
from fastapi.templating import Jinja2Templates
from dotenv import load_dotenv #llm yüklemek için kullanırız
...
def get_db(): #veritabanı bağlantısı oluşturuyoruz
    db=SessionLocal()
    try:
        yield db # yield return gibi ama birden çok değer döndürmek için kullanırız
    finally:
        db.close()
...
async def render_todo_page(request:Request,todo_id:int,db:db_dependency):
    try:
        ...
        todo=db.query(Todo).filter(Todo.owner_id==todo_id).first()
...
async def read_by_id(user:user_dependency,db:db_dependency,todo_id:int=Path(gt=0)):#gt sıfırdan küçük değerleri kabul etmez
    if user is None:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED)
...
def create_todo_with_gemini(todo_string:str):
    load_dotenv()
    genai.configure(api_key=os.environ.get("GOOGLE_API_KEY"))# BİZE VERİLEN APİ KEYİ GİRİYORUZ
